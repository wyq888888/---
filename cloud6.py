...
def _compute_diameters_on_frame(
	...
	for det_index in range(len(xywh)):
		...
		physical_diameter_cm_raw = float(physical_diameter_m * 100.0)
		# 线性回归校正：D_true = 0.8090 * D_measured + 0.7670（MAE=0.109cm）
		CALIB_A = 1.1131
		CALIB_B = -0.6476
		physical_diameter_cm = CALIB_A * physical_diameter_cm_raw + CALIB_B
		...
		while True:
			...
			if len(frame_buf) == STABLE_N and (frame_index - last_stable_frame) >= STABLE_N:
				...
				if all_stable and xyxy is not None:
					...
					# ── 保存标注图为 PNG ────────────────────────────────
					# 左上角叠加每个目标的详细数据，方便与CSV对照核查
					save_img = annotated.copy()
					for di, row in enumerate(rows):
						_, _, rz, rpx, _, rcm, _, rconf = row
						info = f"#{di}  D={rcm:.2f}cm  px={rpx:.1f}  z={rz:.3f}m  conf={rconf:.2f}"
						cv2.putText(save_img, info, (8, 28 + di * 24),
									cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 255), 1, cv2.LINE_AA)
					png_path = output_dir / f"stable_frame{mid_fidx:05d}.png"
					cv2.imwrite(str(png_path), save_img)
					print(f"[SAVE]   标注图已保存: {png_path}")
